		...
		if(!empty($_FILES['Filedata']['tmp_name'])) {
			...
			if($imginfo[2] == 1) {
				...
				$r = image::thumb($file['tmp_name'], $file['tmp_name'].'.jpg', $this->conf['upload_image_max_width'], 240000);
				image::clip_thumb($file['tmp_name'], $file['tmp_name'].'_thumb.jpg', $this->conf['thread_icon_middle'], $this->conf['thread_icon_middle']);

				$md5name = md5($aid.'_'.$this->conf['auth_key']);
				$fileurl = image::get_dir($aid).'/'.$md5name.'.jpg';
				$destfile = '/attach/'.$fileurl;
				$thumbfile = image::thumb_name($destfile);

				$response = $this->attach->bcs_upload($destfile, $file['tmp_name'].'.jpg', array('filename' => $file['name']));
				$response2 = $this->attach->bcs_upload($thumbfile, $file['tmp_name'].'_thumb.jpg', array('filename' => image::thumb_name($file['name'])));
			...
			is_file($file['tmp_name'].'.jpg') && unlink($file['tmp_name'].'.jpg');
			is_file($file['tmp_name'].'_thumb.jpg') && unlink($file['tmp_name'].'_thumb.jpg');
