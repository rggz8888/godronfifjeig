  ...
  function noticeItemSwitch(item, cb){
	...
	function _paint(data){
		...
		for(var i = 0;i < com.length;i ++){
			...
			$.each(tmp, function(j, item){
				if(item.title == com[i].title){	
					tmp.splice(j, 1);
				}
			});
			tmp.unshift(com[i]);
		...
		for(var i = 0;i < tmp.length && i < 5;i ++){
			...
			}else if(tmp[i].title[0] == '^'){
				str = str + '<span class="item"><a href="' + tmp[i].link + '" target="_blank">' + tmp[i].title.substr(1) + '</a></span>';
			}else{
