...
<form action="http://127.0.0.1/user-login-ajax-1.htm" method="post" id="login_form">
	...
	<dl>
		...
		<dd><input type="text" name="email" id="login_email" value="<?php echo isset($user['email']) ? $user['email'] : '';?>" tabindex="10" style="width: 250px;" /> <a href="http://127.0.0.1/user-create-ajax-1.htm" onclick="return false;" class="ajaxdialog" id="login_create_link">注册</a></dd>
		...
		<dd>
			...
			<?php if($conf['resetpw_on']) { ?><a href="http://127.0.0.1/user-resetpw.htm" target="_blank">找回密码</a><?php } ?>
		</dd>
	...
	$('#login_submit_ajax').click(function() {
		...
		$.post($('#login_form').attr('action'), postdata,  function(s){
			...
			if(!json.user) {
				...
				$('#user').html('<a href="http://127.0.0.1/my-profile.htm">' + user.username + '</a> '+adminadd);
