...
!defined('FRAMEWORK_PATH') && exit('FRAMEWORK_PATH not defined.');
	include '/home/xmulifec/domains/xmulife.com/public_html/tmp/control_admin_control.class.php';
...
class plugin_control extends admin_control {
	...
	public function on_list() {
		...
		$pages = misc::simple_pages("http://www.xmulife.com/admin/plugin-list-cateid-$cateid-styleid-$styleid-orderby-$orderby.htm", count($pluginlist), $page, $pagesize);
	...
	public function on_install() {
		...
		if(!is_dir($this->conf['plugin_path'].$dir)) {
			...
			if(IN_SAE) {
				// 提示下载
				$next = "http://www.xmulife.com/admin/plugin-install-dir-$dir.htm";
		...
		$referer = core::gpc('HTTP_REFERER', 'S') OR $referer = 'http://www.xmulife.com/admin/plugin-index.htm';
		if(IN_SAE) {
			// 此处强制刷新一下静态变量 static $plugins = array();
			misc::get_plugins($this->conf, 1);
			
			$this->make_tmp($this->conf);
			$url = "http://www.xmulife.com/admin/plugin-saetmp.htm";
	...
	public function on_upgrade() {
		...
		if(IN_SAE) {
			...
			$saetmp = 'http://www.xmulife.com/admin/plugin-saetmp.htm';
