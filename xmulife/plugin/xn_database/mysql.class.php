...
final class mysql {
	...
	public function connect() {
		$func = 'mysql_connect';
		if(!$this->link = @$func($this->config['host'], $this->config['user'], $this->config['password'], 1)) {
		...
		if($this->config['name'] && !@mysql_select_db($this->config['name'], $this->link)) {
			$this->halt('Cannot use database '.$this->config['name']);
			return false;
		}
